...
**All lessons and reference documents must use a dark theme** — dark background, light text, a single accent colour. This is a global user preference that applies to every topic workspace.
...
- **Compensate for the fixed topnav.** Every topic's `assets/style.css` must include `[id] { scroll-margin-top: 66px; }` (or similar, matched to the topnav's height). Without it, jumping to an anchor lands the target directly under the fixed nav bar, hidden from view.
